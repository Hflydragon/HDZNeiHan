...
-(void)setUpTitleArrays:(NSArray *)array
{
    ...
    if(_titleArray.count>0){
        for(int i=0;i<_titleArray.count;i++){
            ...
            item.frame = CGRectMake(i*60, 0, 60, self.frame.size.height);
    ...
    self.contentSize = CGSizeMake(60*_titleArray.count, self.frame.size.height);
...
-(void)selectedDoSomething:(HDZCustomMenusScrollItem *)label
{
    ...
    if(label.center.x>KscreenWidth/2){
        
        if (label.center.x<self.contentSize.width-KscreenWidth/2) {
            
            [self setContentOffset:CGPointMake((label.center.x-KscreenWidth/2), 0) animated:YES];
        }else{
