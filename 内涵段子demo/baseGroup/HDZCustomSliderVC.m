...
#import "HDZCustomHomeTableController.h"
...
{
    [self.scrollView setContentOffset:CGPointMake(KscreenWidth*selectIndex, 0) animated:YES];
}
...
- (void)addChildViewControllers:(UIViewController *)viewController AtIndex:(NSInteger)index
{
    ...
    viewController.view.backgroundColor = [UIColor colorWithRed:arc4random()%255/255.0 green:arc4random()%255/255.0 blue:arc4random()%255/255.0 alpha:1.0];
    viewController.view.frame = CGRectMake(KscreenWidth*index, 0, KscreenWidth, self.scrollView.frame.size.height);
...
- (void)getShowTheFonrt:(NSInteger)index
{
    ...
    if (viewController==nil) {
        HDZCustomHomeTableController *base = [[HDZCustomHomeTableController alloc] init];
        [self addChildViewControllers:base AtIndex:index];
    }else{
